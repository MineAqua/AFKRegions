...
public class ModernAdapter implements VersionAdapter {
  ...
  public void onEnter(org.bukkit.entity.Player p, com.afkregions.model.Region r) {
    ...
    if (titlesEnabled) {
      ...
      try { p.resetTitle(); } catch (Throwable ignored) {}
...
// ====== TITLES desde lang.yml (progress_title) + PAPI ======
  ...
  @Override public void clearUI(Player p) {
    BossBar bar = bars.remove(p.getUniqueId());
    if (bar != null) bar.removeAll();
  }

  @Override public void shutdown() { for (BossBar b : bars.values()) b.removeAll(); bars.clear(); }

  private static BarColor parseColor(String s) { try { return BarColor.valueOf(s.toUpperCase(Locale.ROOT)); } catch (Exception e) { return BarColor.BLUE; } }
  private static BarStyle parseStyle(String s) { try { return BarStyle.valueOf(s.toUpperCase(Locale.ROOT)); } catch (Exception e) { return BarStyle.SOLID; } }
  private static String applyPAPI(Player p, String s) {
    if (s == null) return "";
    if (org.bukkit.Bukkit.getPluginManager().isPluginEnabled("PlaceholderAPI")) {
      try { return me.clip.placeholderapi.PlaceholderAPI.setPlaceholders(p, s); }
