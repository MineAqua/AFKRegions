...
public class LegacyAdapter implements VersionAdapter {
  ...
  private static String toJsonColored(String s) {
    String colored = org.bukkit.ChatColor.translateAlternateColorCodes('&', s);
    net.md_5.bungee.api.chat.BaseComponent[] comps =
            net.md_5.bungee.api.chat.TextComponent.fromLegacyText(colored);
    return net.md_5.bungee.chat.ComponentSerializer.toString(comps);
  }

  private void sendPacket(Player p, Object packet) throws Exception {
    String v = p.getServer().getClass().getPackage().getName().split("\\.")[3];
    Object handle = p.getClass().getMethod("getHandle").invoke(p);
    Object conn = handle.getClass().getField("playerConnection").get(handle);
    Method sendPacket = conn.getClass().getMethod("sendPacket", Class.forName("net.minecraft.server."+v+".Packet"));
