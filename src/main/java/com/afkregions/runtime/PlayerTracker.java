...
public class PlayerTracker implements Listener {
  ...
  private void debug(String s) { if (debug) plugin.getLogger().info("[DEBUG] " + s); }
...
// 1) calcula todos los segundos "at" cruzados en este tick
  ...
  public void onMove(PlayerMoveEvent e) {
    ...
    if (t == null) return;
    if (f.getWorld() != t.getWorld()) { onBlockChange(e.getPlayer(), t); return; }
  ...
  private void onBlockChange(Player p, Location to) {
    ...
    for (Region r : plugin.regions().candidates(world, bx, bz)) {
      if (r.contains(world, bx, by, bz)) { current = r; break; }
    }
  ...
  public String placeholder(UUID id, String key) {
    ...
    switch (key) {
      case "region":         return st.region.name;
      case "elapsed":        return String.valueOf(elapsedSec);

      // NUEVOS que pide PAPI (por si alguien llama vía fallback):
      case "duration":       return String.valueOf(durationSeconds(id));
      case "progress":       return String.valueOf(progressPercent(id));
      case "progress_bar":   return progressBar(id);
      case "time_left":      return String.valueOf(timeLeftSeconds(id));

      // alias opcional (puedes borrarlo si no lo usas)
      case "progress_percent": return String.valueOf(progressPercent(id));
  ...
  public boolean isAfk(java.util.UUID id) {
    State st = states.get(id); return st != null && st.region != null; }

  public String regionName(java.util.UUID id) {
    State st = states.get(id); return st != null && st.region != null ? st.region.name : ""; }

  public int elapsedSeconds(java.util.UUID id) {
    State st = states.get(id); return st != null && st.region != null ? (st.elapsed/20) : 0; }
  ...
  private static class State {
    final Player player; Region region; int elapsed; java.util.Set<Integer> firedSeconds = new java.util.HashSet<>();
    State(Player p) { this.player = p; this.region = null; this.elapsed = 0; }
  }
}
